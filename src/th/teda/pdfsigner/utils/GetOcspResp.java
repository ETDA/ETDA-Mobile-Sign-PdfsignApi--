...
public class GetOcspResp {
	...
	public OCSPResp getOcspResp(X509Certificate certificate, X509Certificate issuer) throws MalformedURLException,
			IOException, CertificateEncodingException, OperatorCreationException, OCSPException, Exception {

		Principal subjectX500Principal = certificate.getSubjectX500Principal();

		String ocspUrl = getOCSPUrl(certificate);
		if (ocspUrl == null) {
			throw new Exception("OCSP URL for '" + subjectX500Principal + "' is empty");
		}
	...
	public OCSPResp requestOCSPResponse(String url, OCSPReq ocspReq)
...
//			  System.out.println("OCSP request has been failed (HTTP {}) - {} " +
//			  connection.getResponseCode() + connection.getResponseMessage());
	...
	//@SuppressWarnings("resource")
	public X509Certificate getIssuerCert(X509Certificate certificate) throws IOException, Exception {
		byte[] obj;
		obj = certificate.getExtensionValue(Extension.authorityInfoAccess.getId());
		...
		AuthorityInformationAccess authorityInformationAccess;
		DEROctetString oct = (DEROctetString) (new ASN1InputStream(new ByteArrayInputStream(obj)).readObject());
		ASN1InputStream octStream = new ASN1InputStream(oct.getOctets());
		authorityInformationAccess = AuthorityInformationAccess
				.getInstance(octStream.readObject());
